        ...
        // When lower Quantity is clicked
            ...
            addItem.forEach(item => {                
                if (item.FoodID === id && item.Quantity >= 1){
                    item.Quantity -= 1;
                    if (item.Quantity === 0){
                        if ( window.location != 'https://coolfool112.github.io/Food-In-Instant/user-orders.html'){
...
function clearUserCart(addItem, addToCartBtn, trimedEmailID){
    ...
    cartValues.forEach(values => { values.innerHTML = '0';})
    cartTotal.innerHTML = '0';
    if ( window.location != 'https://gcoolfool112.github.io/Food-In-Instant/user-orders.html'){
        // Enable removed items btn
        addItem.forEach(item=>{
            addToCartBtn[item.FoodID-1].disabled = false;
            addToCartBtn[item.FoodID-1].innerHTML = 'Add to Cart';
...
function ClientDataFlow(addToCartBtn){
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            checkOutBtn.addEventListener('click', ()=>{
                ...
                if (addItem.length != 0){
                    ...
                    window.setTimeout(function(){
                        window.location.replace('https://coolfool112.github.io/Food-In-Instant/user-orders.html')
                    }, 2600)
                } else {
                    Swal.fire({
                        icon: 'error',
            ...
            // Shows Orders
            if (window.location.href === 'https://coolfool112.github.io/Food-In-Instant/user-orders.html'){
